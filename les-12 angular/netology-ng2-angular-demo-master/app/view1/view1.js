...
angular.module('myApp.view1', ['ngRoute'])
    ...
    .controller('View1Ctrl', function ($scope) {
        ...
        $scope.alertPokesName = (pokemon) => {
            alert('Имя покемона: ' + pokemon.name);
        };

        $scope.typeFilter = (pokemon) => {
            return pokemon.type.some(
                pokestype => pokestype == $scope.seacrhByType );
        };


        $scope.customFilter = (pokemon) => {
            if($scope.searchQuery) {
                if(pokemon.name.toLowerCase().indexOf($scope.searchQuery.toLowerCase()) !== -1) {
                    if ($scope.seacrhByType) {
                        return $scope.typeFilter(pokemon);
                    }
                    return true;
                }
            }

            if (!$scope.searchQuery && $scope.seacrhByType) {
                return $scope.typeFilter(pokemon);
            }

            if(!$scope.seacrhByType && !$scope.searchQuery) return true;
